...
public:
	inline void forward(Graph* cg, const Instance& inst, HyperParams* hyparams){
		...
		left_concat_labelFeat.forward(cg, &emotion_label, &words_left_grnn._rnn_nodes[word_size - 1]);
		right_concat_labelFeat.forward(cg, &emotion_label, &words_right_grnn._rnn_nodes[0]);
