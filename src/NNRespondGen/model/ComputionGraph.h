		...
		while (true){			
			...
			for (int idx = 0; idx < lastStates.size(); idx++){
				...
				for (int idy = 0; idy < actions.size(); ++idy) {
					scored_action.ac.set(actions[idy]);
			...
			for (int idx = 0; idx < offset; idx++){				
				...
				if (action.isSeparate() || action.isFinish()) {
					states[step][idx]._inc_nodes.word_lstm = &(pGenerator->_nextscores.word_lstm[beam[idx].position]);
				}
				else {
					states[step][idx]._inc_nodes.word_lstm = pGenerator->_inc_nodes.word_lstm;
				}
